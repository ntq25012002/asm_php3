...
class RoomController extends Controller
{
    ...
    public function roomList(Request $request) {
        ...
        if(!empty($request->checkin)){
            ...
            $filterBookings['checkin'] = $checkin;
            if(empty($request->checkout)){
                $filterBookings['checkout'] = '';
            }
            // $filterBookings[] = ['checkin','LIKE','%'.$checkin.'%'];
        }
        if(!empty($request->checkout)){
            $checkout = $request->checkout;
            $filterBookings['checkout'] = $checkout;
            if(empty($request->checkin)){
                $filterBookings['checkin'] = '';
            }
        ...
        if(count($filterBookings) > 0 || count($filters)){
            $ids = [];
            $roomsBookings = $this->bookings->search($filterBookings);
            foreach($roomsBookings as $item) {
                $ids[] = $item->room_id;
            }
            $rooms1 = $this->rooms->search($ids,$filters);
        }else {
