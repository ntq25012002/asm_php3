...
class RoomTypeController extends Controller
{
    ...
    public function index() {
        $list = $this->roomTypes->loadList()->toArray();
        $roomTypes = [];
        $results = [];
        $ids = [];
        ...
        // foreach($list as $item) {
        //     foreach($list as $j) { 
        //         if($item->room_type_id == $j->room_type_id) {
        //             $results = array_push($j,$results);
        //             $idItem = array_search($j,$list);
        //         }
        //     }
        ...
        for ($i=0; $i < count($list); $i++) { 
            // dd($list[$i]->room_type_id);
            for ($j=0; $j < count($list); $j++) { 
                if($list[$i]->room_type_id == $list[$j]->room_type_id) {
                // $idItem = array_search($list[$i],$list);
                $results = array_push($list[$i],$results);
