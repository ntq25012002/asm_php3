...
class StaffController extends Controller
{
    ...
    //     }else{
    //         $staffs = $queryStaff->where('role_id','<>',4)->get();
    //     }
    //     if(count($staffs) > 0) {
    ...
    //         $output = '<tr>
    //                         <td class="center text-primary" colspan=9> Không tìm thấy giá trị phù hợp </td>
    //                     </tr>';
    //     }
    ...
    public function deleteStaffs(Request $request) {
        // dd($request->all());
        $staffIds = $request->all();


        $this->staff->whereIn($staffIds)->delete();
