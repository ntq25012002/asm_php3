...
class HomeController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        if(!empty($request->checkin)){
            ...
            $filters['checkin'] = ['checkin','LIKE','%'.$checkin.'%'];
        }
        if(!empty($request->checkout)){
            $checkout = $request->checkout;
            $filters['checkout'] = ['checkout','LIKE','%'.$checkout.'%'];
        }
        if(!empty($request->room_type)){
            $room_type_id = $request->room_type;
            $filters['room_type_id'] = ['room_type_id',$room_type_id];
        }
        if(!empty($request->price)){
            $price = explode('-',$request->price);
            if(count($price) == 1) {
                $filters['price'] = ['price','>=',(int)$price[0]];
            }else if(count($price) == 2) {
                $filters['price'] = ['price','>',(int)$price[0]];
                $filters['price'] = ['price','<',(int)$price[1]];
            }
        }
        ...
        if(count($filters) == 0) {
            ...
            $rooms1 = $this->bookings->whereBetween('checkin',[$filters['checkin'],$filters['checkout'] != ''?$filters['checkout']:''])->where([['deleted_at','<>',null]])
