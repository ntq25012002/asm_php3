...
class RoomTypeController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        if(!empty($request->equipmentIds) && !empty($request->quantity_equipments)) {
            foreach($request->equipmentIds as $item) {
                $equipmentIds[] = $item;
            }

            // for($i = 0; $i< $request->quantity_equipments->length(); $i++) {
            //     dd()
            // }
            foreach($request->quantity_equipments as $quantity) {
                $roomTypeEquipments[] =  [
                                            'room_type_id' => $roomTypeId,
                                            'equipment_id' => $item,
                                            'quantity_equipment' => $quantity
                                        ];
            }
        }
        // if(!empty($request->quantity_equipments)) {
        //     foreach($request->quantity_equipments as $item) {
        //         $quantity_equipments['quantity_equipment'] =  $item;
        //     }
        // }
        dd($request->quantity_equipments->length);
        // $roomTypeNew->equipments()->attach($equipmentIds,['quantity_equipment' => 2]);
