...
class StaffController extends Controller
{
    ...
    public function edit($id) {
        $staff = $this->staffs->where('id',$id)->first();
        // dd($staff);
    ...
    public function update(StaffRequest $request, $id) {
        // dd($request->all());
        $staffOld = $this->staffs->where('id', $id)->first();
        if($request->hasFile('avatar')) {
            $file = $request->file('avatar');
            $fileName = $file->getClientOriginalName();
            $fileNameHash = str::random(20) . '.' . $file->getClientOriginalExtension();
        ...
        $this->staffs->where('id',$id)->update($data);
    ...
    public function destroy($id) {
        try {
            $this->staffs->where('id', $id)->delete();
    ...
    public function deleteStaffs(Request $request) {
        ...
        try {
            $ids = $request->ids;
            $this->staff->whereIn('id', $ids)->delete();
            return response()->json([
                'code'=> 200,
                'message'=> 'success'
            ],200);
