...
class BookingController extends Controller
{
    ...
    public function getBooking(Request $request) {
        ...
        if(Session::has('roomBooking')) {
            ...
            // {{$checkout['tm_mday']}}
            // $this->v['checkinD'] = $datediffCheckin['tm_mday'];
            // $this->v['checkoutD'] = $datediffCheckout['tm_mday'];
            // $this->v['checkinDW'] = $datediffCheckin['tm_wday'] == 0? 'Chủ nhật':'Thứ '.$datediffCheckin['tm_wday']+1;
            // $this->v['checkoutDW'] = $datediffCheckout['tm_wday'] == 0? 'Chủ nhật':'Thứ '.$datediffCheckout['tm_wday']+1;
            ...
            DB::beginTransaction();
                ...
                if($res > 0) {
                    if(Session::has('roomBooking')) {
                        ...
                        if($resBook > 0) {
                            return redirect()->route('confirmatiom')->with('msg','Đặt phòng thành công!');
                        }else{
                            return redirect()->route('booking')->with('err','Lỗi đặt phòng');
                        } 
                    }else{
                        return redirect()->route('booking')->with('err','Chưa có thông tin đặt phòng');
                    }
                }
            DB::commit();
