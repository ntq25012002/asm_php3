...
class RoomTypes extends Model
{
    use HasFactory;
    use SoftDeletes;
    protected $fillable = ['name','image','description','adults','children','equipments.name as name_equipment','room_type_equipment.quantity_equipment'];
    ...
    public function loadList($params = []) {
        $query = DB::table('room_type_equipment')
                // ->select($this->fillable)
                ->select('room_types.*','equipments.8','equipments.name as name_equipments','room_type_equipment.quantity_equipment')
                ->join($this->table,'room_type_equipment.room_type_id','=','room_types.id')
