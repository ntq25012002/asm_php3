...
class RoomTypes extends Model
{
    ...
    protected $fillable = ['room_types.*','equipments.name as equipment','room_type_equipment.quantity_equipment'];
    ...
    public function loadList($params = []) {
        ...
        $query = DB::table($this->table)
             ->select('room_types.*','equipments.name as equipment','room_type_equipment.quantity_equipment')
             ->join('equipments as e', 'room_type_equipment.equipment_id', '=', 'e.id')
             ->join('room_type_equipment','room_type_equipment.room_type_id','=','room_types.id')
             ->where('room_types.deleted_at',null);
        $list = $query->get()->toArray();

        foreach ($list as $item) {
            $item->room_type_equipment = DB::table('room_type_equipment')->where('id',$item->id)->get()->toArray();
        }
        // return $this->belongsToMany(Equipments::class,'room_type_equipment','room_type_id');
        return $list;
