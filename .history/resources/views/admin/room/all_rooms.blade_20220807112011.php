                                ...
                                <form action="" method="get">
                                    <div class="row pr-0">
                                        <select name="status" id="show_role" style="width:150px" class="form-control col-3  mx-2">
                                            ...
                                            <option value="0" {{request()->status==0?'selected':''}}>Bảo trì</option>
                                            <option value="1" {{request()->status==1?'selected':''}}>Hoạt động</option>
                                        </select>
                                        <select name="room_type" id="show_role" style="width:150px" class="form-control col-3  mx-2">
                                            <option value="">--- Loại phòng ---</option>
                                            @foreach ($roomTypes as $item)
                                                <option value="{{$item->id}}" {{request()->room_type==$item->id?'selected':''}}>{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                        <select name="price" id="show_role" style="width:150px" class="form-control col-3  mx-2">
                                            <option value="">--- Giá ---</option>
                                            <option value="0-500000" {{request()->price==500000'selected':''}}>Dưới 500.000đ</option>
                                            <option value="500000-1000000" {{request()->price==$item->id?'selected':''}}>500.000 - 1.000.000đ</option>
                                            <option value="1000000-1500000" {{request()->price==$item->id?'selected':''}}>500.000 - 1.000.000đ</option>
                                            <option value="2000000" {{request()->price==$item->id?'selected':''}}>Trên 2.000.000đ</option>
                                        </select>
