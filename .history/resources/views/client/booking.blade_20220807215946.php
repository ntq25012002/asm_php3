    ...
    <div class="container">
        <div class="booking">
            <div class="row">
                ...
                <p>{{$checkin['tm_mon']+1}}/{{$checkin['tm_year'] + 1900}}<br>{{$checkin['tm_wday'] == 0? 'Chủ nhật':'Thứ '.$checkin['tm_wday']+1}}</p>
                </div>
                <div class="rd-box">
                <label>Check out</label>
                <p class="bold">{{$checkout['tm_mday']}}</p>
                <p>{{$checkout['tm_mon']+1}}/{{$checkout['tm_year'] + 1900}}<br>{{$checkout['tm_wday']+1 == 8? 'Chủ nhật':'Thứ '.$checkout['tm_wday']+1}}</p>
                </div>
                </div>
                <div class="rd-top">
                <div class="rd-box">
                ...
                <div class="rd-box">
                <p class="bold">{{Session::get('roomBooking')['nights']}}</p>
                <p>Đêm</p>
                </div>
                </div>
                </div>
                <table class="reservation-table table">
                    <tbody>
                        <tr>
                            <td>1 Phòng x {{Session::get('roomBooking')['nights']}} Đêm</td>
                            <td>{{number_format(Session::get('roomBooking')['nights'] * $room->price,'0',',','.')}}đ</td>
                        </tr>
                        {{-- <tr>
                            <td>Tax</td>
                            <td>$80</td>
                ...
                <tfoot>
                    <tr>
                        <td>Tổng</td>
                        <td>{{number_format(Session::get('nights') * $room->price,'0',',','.')}}đ</td>
                    </tr>
                </tfoot>
                </table>
                </div>
