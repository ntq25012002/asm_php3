...
input.addEventListener('change', () => {
    ...
    for (let j = 0; j < arrDataImages.length; j++) {
      const item = arrDataImages[j];
      
      if (files.every(e => e.name !== file[i].name) && file[i].name !== item) {
        files.push(file[i])
      } else if(files.every(e => e.name === file[i].name) || file[i].name === item) {
        console.log('đã tồn tại file ảnh');
      }
